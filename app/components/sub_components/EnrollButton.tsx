...
import { enroll as enrollCourse, unenroll as unenrollCourse } from '@/app/store/coursesSlice';
...
function EnrollButton({ courseId, isEnrolled: initialEnrolled }: ExtendedEnrollButtonProps) {
  ...
  const [enrolled, setEnrolled] = useState(initialEnrolled); // FIX: was inverted
...
const handleEnrollToggle = async () => {
  ...
  try {
    ...
    Swal.fire({
      icon: 'success',
      title: 'Success!',
      text: nextEnrollState ? 'The course has been successfully enrolled.' : 'You have been successfully unenrolled from the course.',
    ...
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: user ? 'Something went wrong. Please try again.' : 'You must be logged in!',
  ...
  return (
    <button
      ...
      className={`px-6 py-2 rounded-md text-white transition font-semibold
        ${enrolled ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}
