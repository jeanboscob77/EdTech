...
import { NextResponse } from "next/server";
 ...
 export  const  POST = async(req: Request)=>{
   await connectDB()
    ...
    try {
        if (!names || !email || !subject|| !message) return NextResponse.json({message: 'All fields required'},{status: 400})
 ...
 export  const  GET = async(req: Request)=>{
   ...
   const contacts = await Contacts.find()
    ...
    try {
      if(contacts.length === 0) return NextResponse.json({msg: "no contacts founds"},{status: 404})
